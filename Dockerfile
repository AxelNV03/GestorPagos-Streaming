...
RUN echo "0 3 * * * /app/backup_auto.sh >> /var/log/backup.log 2>&1" > /etc/cron.d/backup-cron && \
    echo "0 9 1 * * /app/recordatorio_mensual.sh >> /var/log/recordatorio.log 2>&1" >> /etc/cron.d/backup-cron && \
    echo "0 9 6 * * /app/recordatorio_vencido.sh >> /var/log/recordatorio.log 2>&1" >> /etc/cron.d/backup-cron && \
